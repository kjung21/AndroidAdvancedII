...
import com.kryptopass.treasurehunt.databinding.ActivityHuntMainBinding
 ...
 * This app demonstrates how to create and remove geofences using the GeofencingApi.
 * Uses an BroadcastReceiver to monitor geofence transitions and creates notification
 * and finishes the game when the user enters the final geofence (destination).
 * This app requires a device's Location settings to be turned on.
 * It also requires the ACCESS_FINE_LOCATION permission and user consent.
 * For geofences to work in Android Q,
 * app also needs the ACCESS_BACKGROUND_LOCATION permission and user consent.
 */
...
class HuntMainActivity : AppCompatActivity() {
     ...
     * we call checkDeviceLocationSettingsAndStartGeofence again to make sure it's actually on,
    ...
    /*
     * When the user clicks on the notification, this method will be called,
     * letting us know that the geofence has been triggered,
     * and it's time to move to the next one in the treasure hunt.
     */
    ...
    /**
     * This will also destroy any saved state in the associated ViewModel,
     * so we remove the geofences here.
     */
    ...
    /**
     * Starts the permission check and Geofence process only if the Geofence associated
     * with the current hint isn't yet active.
     */
    ...
    private fun checkDeviceLocationSettingsAndStartGeofence(resolve:Boolean = true) {
        val locationRequest = LocationRequest.Builder(Priority.PRIORITY_LOW_POWER, 1000).apply {
...
//        }
     ...
     * across Android 10+ and all other Android versions.
     */
    @TargetApi(29)
     ...
     * This method should be called after the user has granted the location permission.
     * If there are no more geo-fences, we remove the geofence and let the viewmodel know
     * that the ending hint is now "active".
     */
